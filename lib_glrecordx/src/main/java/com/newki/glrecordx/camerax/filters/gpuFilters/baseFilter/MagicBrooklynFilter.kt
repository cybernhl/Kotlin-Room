...
class MagicBrooklynFilter :
    GPUImageFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.brooklyn)) {
    ...
    public override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] =
                loadTexture(getApplicationContext(), "filter/brooklynCurves1.png")
            inputTextureHandles[1] =
                loadTexture(getApplicationContext(), "filter/filter_map_first.png")
            inputTextureHandles[2] =
                loadTexture(getApplicationContext(), "filter/brooklynCurves2.png")
        }
    }
}