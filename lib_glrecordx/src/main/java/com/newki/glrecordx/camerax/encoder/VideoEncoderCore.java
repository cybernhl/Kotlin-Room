...
public class VideoEncoderCore {
    ...
    //音频配置
    private String audioMime = MIMETYPE_AUDIO_AAC ;   //音频编码的Mime - aac格式
    ...
    public VideoEncoderCore(int width, int height, int bitRate, String path) throws IOException {
        ...
        mAudioEncoder = MediaCodec.createEncoderByType(audioMime); //创建音频编码器
        mAudioEncoder.configure(aFormat, null, null, MediaCodec.CONFIGURE_FLAG_ENCODE); //配置
        ...
        mRecorder = new AudioRecord(MediaRecorder.AudioSource.MIC, sampleRate, channelConfig, audioFormat, bufferSize); //初始化录音器
        mAudioEncoder.start();        //开启音频编码
        mRecorder.startRecording();   //直接开启录音器

        //视频编码的配置
        mBufferInfo = new MediaCodec.BufferInfo();
        MediaFormat format = MediaFormat.createVideoFormat(MIME_TYPE, width, height);
        //COLOR_FormatYUV420SemiPlanar //一般都是选择NV21，这里因为要滤镜效果，需要滤镜处理后的图像直接编码
        ...
        mVideoEncoder.start();      // 开启视频编码

        // 封装格式混合器
        mMuxer = new MediaMuxer(path, MediaMuxer.OutputFormat.MUXER_OUTPUT_MPEG_4);  //封装格式MP4
...
//                    encodedData.limit(mBufferInfo.offset + mBufferInfo.size);
    ...
    class AudioEncoder implements Runnable {
        ...
        private boolean audioStep() throws IOException {
            ...
            if (index >= 0) {
                ...
                if (length > 0) {
                    if (baseTimeStamp != -1) {
                        ...
                        mAudioEncoder.queueInputBuffer(index, 0, length, time, isRecording ? 0 : MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                    } else {
                        mAudioEncoder.queueInputBuffer(index, 0, length, 0, isRecording ? 0 : MediaCodec.BUFFER_FLAG_END_OF_STREAM);
                    }
                }
            }
            ...
            int outIndex;
            do {
                outIndex = mAudioEncoder.dequeueOutputBuffer(mInfo, 0);
                if (outIndex >= 0) {
                    ...
                    ByteBuffer buffer = getOutputBuffer(mAudioEncoder, outIndex);
