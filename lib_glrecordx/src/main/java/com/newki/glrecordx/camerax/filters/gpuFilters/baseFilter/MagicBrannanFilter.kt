...
class MagicBrannanFilter :
    GPUImageFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.brannan)) {
    ...
    override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] =
                loadTexture(getApplicationContext(), "filter/brannan_process.png")
            inputTextureHandles[1] =
                loadTexture(getApplicationContext(), "filter/brannan_blowout.png")
            inputTextureHandles[2] =
                loadTexture(getApplicationContext(), "filter/brannan_contrast.png")
            inputTextureHandles[3] = loadTexture(getApplicationContext(), "filter/brannan_luma.png")
            inputTextureHandles[4] =
                loadTexture(getApplicationContext(), "filter/brannan_screen.png")
        }
    }
}