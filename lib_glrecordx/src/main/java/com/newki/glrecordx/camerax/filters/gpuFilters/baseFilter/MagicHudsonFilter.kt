...
class MagicHudsonFilter :
    GPUImageFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.hudson)) {
    ...
    override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] =
                loadTexture(getApplicationContext(), "filter/hudsonbackground.png")
            inputTextureHandles[1] = loadTexture(getApplicationContext(), "filter/overlaymap.png")
            inputTextureHandles[2] = loadTexture(getApplicationContext(), "filter/hudsonmap.png")
        }
    }
}