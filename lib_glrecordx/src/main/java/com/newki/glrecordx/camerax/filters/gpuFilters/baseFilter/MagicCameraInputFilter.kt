...
class MagicCameraInputFilter : GPUImageFilter(
    readShaderFromRawResource(R.raw.default_vertex),
    readShaderFromRawResource(R.raw.default_fragment)
) {
    ...
    override fun onDrawFrame(textureId: Int): Int {
        ...
        GLES20.glUniformMatrix4fv(
            mTextureTransformMatrixLocation,
            1,
            false,
    ...
    fun onDrawToTexture(textureId: Int): Int {
        if (mFrameBuffers == null) return OpenGlUtils.NO_TEXTURE
        runPendingOnDrawTasks()
    ...
    fun initCameraFrameBuffer(width: Int, height: Int) {
        if (mFrameBuffers != null && (mFrameWidth != width || mFrameHeight != height)) destroyFramebuffers()
