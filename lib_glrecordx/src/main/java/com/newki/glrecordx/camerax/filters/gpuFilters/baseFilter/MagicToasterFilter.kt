...
class MagicToasterFilter : GPUImageFilter(
    NO_FILTER_VERTEX_SHADER,
    readShaderFromRawResource(R.raw.toaster2_filter_shader)
) {
    ...
    public override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] = loadTexture(getApplicationContext(), "filter/toastermetal.png")
            inputTextureHandles[1] =
                loadTexture(getApplicationContext(), "filter/toastersoftlight.png")
            inputTextureHandles[2] =
                loadTexture(getApplicationContext(), "filter/toastercurves.png")
            inputTextureHandles[3] =
                loadTexture(getApplicationContext(), "filter/toasteroverlaymapwarm.png")
            inputTextureHandles[4] =
                loadTexture(getApplicationContext(), "filter/toastercolorshift.png")
        }
    }
}