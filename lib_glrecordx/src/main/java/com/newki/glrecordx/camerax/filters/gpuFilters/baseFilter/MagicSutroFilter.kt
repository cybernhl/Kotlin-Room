...
class MagicSutroFilter :
    GPUImageFilter(NO_FILTER_VERTEX_SHADER, readShaderFromRawResource(R.raw.sutro)) {
    ...
    public override fun onInitialized() {
        ...
        runOnDraw {
            inputTextureHandles[0] = loadTexture(getApplicationContext(), "filter/vignette_map.png")
            inputTextureHandles[1] = loadTexture(getApplicationContext(), "filter/sutrometal.png")
            inputTextureHandles[2] = loadTexture(getApplicationContext(), "filter/softlight.png")
            inputTextureHandles[3] =
                loadTexture(getApplicationContext(), "filter/sutroedgeburn.png")
            inputTextureHandles[4] = loadTexture(getApplicationContext(), "filter/sutrocurves.png")
        }
    }
}