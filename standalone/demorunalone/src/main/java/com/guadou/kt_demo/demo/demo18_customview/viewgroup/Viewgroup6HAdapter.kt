 ...
 * 测试的数据适配器
 */
class Viewgroup6HAdapter(private val data: List<String>) : CurtainAdapter() {

    override fun getItemCount(): Int = data.size

    override fun getItemViewType(position: Int): Int {
        return if (position == 1 || position == 8 ||
            position == 15 || position == 22
    ...
    override fun onCreateItemView(context: Context, parent: ViewGroup, itemType: Int): View {
        return if (itemType == 5) {
            LayoutInflater.from(context).inflate(R.layout.item_img_group6h_2, parent, false)
        } else {
            LayoutInflater.from(context).inflate(R.layout.item_img_group6h, parent, false)
        }
    }
