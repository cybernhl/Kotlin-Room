...
public abstract class BasePopupView extends FrameLayout implements OnNavigationBarListener {
    ...
    protected void applySize(boolean isShowNavBar) {
        ...
        //-- Newki修改，添加自定义的判断，如果添加Popup的容器不是decorView，那么不需要添加NavBarHeight --
        final Activity activity = (Activity) getContext();
        ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
        int decorViewId = decorView.getId();
        int groupId = popupInfo.decorView.getId();

        boolean isNavBarShown = isShowNavBar || (XPopupUtils.isNavBarVisible(getContext()) && decorViewId == groupId);
